...
import in.com.v2kart.commercewebservices.populator.HttpRequestAddressDataPopulator;
import in.com.v2kart.commercewebservices.populator.HttpRequestCustomerDataPopulator;
import in.com.v2kart.commercewebservices.populator.options.PaymentInfoOption;
import in.com.v2kart.commercewebservices.user.data.AddressDataList;
import in.com.v2kart.commercewebservices.user.data.LoginChangeResponse;
import in.com.v2kart.commercewebservices.user.data.PasswordRestoreResponse;
import in.com.v2kart.commercewebservices.user.data.UserWalletData;
import in.com.v2kart.commercewebservices.validation.data.AddressValidationData;
import in.com.v2kart.commercewebservices.wishlist.WishListDataList;
...
public class CustomersController extends BaseController {
    ...
    private V2CustomerFacade customerFacade;
    ...
    /**
     * Client should pass customer's data as POST Body. Content-Type needs to be set to application/x-www-form-urlencoded; charset=UTF-8 and
     * sample body (urlencoded) is: old=1234&new=1111<br>
    ...
    public void registerUser(@RequestParam final String login, @RequestParam final String password,
            ...
            @RequestParam final String lastName, @RequestParam final String mobileNumber) throws DuplicateUidException,
        ...
        if (!EmailValidator.getInstance().isValid(login)) {
            throw new RequestParameterException("Login [" + login + "] is not a valid e-mail address!", RequestParameterException.INVALID,
                    "login");
        }
        ...
        // hardCoding titleCode
        registration.setTitleCode("ms");
        registration.setMobileNumber(mobileNumber);
        customerFacade.register(registration);
    }

    /**
     * Creates the user dedicated to do a guest checkout. Newly created user will be available only by cartService and only during the
     * current session.
     *
     * @param email
    ...
    /**
     * Converts currently logged guest user to full user. It is possible to call it after guest user has placed order during the same
     * session.
     *
     * @param password
     * @param httpSession
    ...
    @ResponseBody
    @ResponseStatus(value = HttpStatus.OK)
    public AddressDataList updateDefaultAddress(@PathVariable final String id) throws DuplicateUidException {
        if (LOG.isDebugEnabled()) {
            LOG.debug("updateDefaultAddress: id=" + id);
        }
        final AddressDataList addressDataList = new AddressDataList();

        final de.hybris.platform.commercefacades.user.data.AddressData address = userFacade.getAddressForCode(id);
        if (address == null) {
     ...
     */
    @Secured("ROLE_CUSTOMERGROUP")
    @RequestMapping(value = "/current/profile", method = { RequestMethod.PUT, RequestMethod.POST })
    @ResponseBody
    ...
    public AddressData createAddress(final HttpServletRequest request) throws DuplicateUidException, WebserviceValidationException {
        ...
        /*
         * //hardCoding title addressData.setTitleCode("ms");
         */
        userFacade.addAddress(addressData);

        final List<AddressData> addressDatas = userFacade.getAddressBook();

        // If addressbook has the above added address then set it as the DefaultAddress.
        if (null != addressDatas && addressDatas.size() == 1) {
            userFacade.setDefaultAddress(addressData);
            addressData.setDefaultAddress(true);
        }

        // if (addressData.isMakeDefault())
    ...
    @ResponseBody
    public AddressData editAddress(@PathVariable final String id, final HttpServletRequest request) throws WebserviceValidationException {
        if (LOG.isDebugEnabled()) {
        ...
        if (address.getId().equals(userFacade.getDefaultAddress().getId()) && address.isDefaultAddress()) {
    ...
    /**
     * Client should pass old and new password in Body. Content-Type needs to be set to application/x-www-form-urlencoded; charset=UTF-8 and
     * sample body (urlencoded) is: old=1234&new=1111
     *
     * @param old
     *        - old password
     * @param newPassword
    ...
    /**
     * Client should pass old and new password in Body. Content-Type needs to be set to application/x-www-form-urlencoded; charset=UTF-8 and
     * sample body (urlencoded) is: new=1111
     *
     * @param newPassword
     *        - new password
     */
    ...
    /**
     * This method gets the wishlist data
     *
     * @param request
    ...
    @Secured("ROLE_CUSTOMERGROUP")
    @ResponseBody
    public WishListDataList getWishlist() {
        final WishListDataList dataList = new WishListDataList();
        final List<WishlistData> wishlistData = wishlistFacade.getWishlists();
        dataList.setWhislists(wishlistData);
        return dataList;
    }

    @RequestMapping(value = "/remove-wishlist", method = RequestMethod.GET)
    @Secured("ROLE_CUSTOMERGROUP")
    @ResponseBody
    public WishListDataList removeWishlistItem(@RequestParam("productCode") final String productCode) {
        final WishListDataList dataList = new WishListDataList();
    ...
    @Secured("ROLE_CUSTOMERGROUP")
    @ResponseBody
    public UserWalletData getWallet() {
        final UserWalletData userWalletData = new UserWalletData();
        userWalletData.setTransactions(v2CustomerWalletFacade.getCustomerTransactions());
        userWalletData.setCustomerBalance(v2CustomerWalletFacade.getBalanceForCustomer());
        return userWalletData;
    }

    @Secured("ROLE_CUSTOMERGROUP")
    @ResponseBody
    @RequestMapping(value = "/add-cart-wishlist", method = RequestMethod.GET)
    public WishListDataList addToCart(@RequestParam("productCode") final String productCode) {
        long actualQuantity = 0;

        actualQuantity = addItemtoCart(1, productCode, actualQuantity);

        if (actualQuantity != 0) {
            wishlistFacade.removeWishlistEntryData(productCode);
            // GlobalMessages.addFlashMessage(redirectAttributes, GlobalMessages.CONF_MESSAGES_HOLDER, "wishlist.items.cart.added");
        }
     ...
     */
    private void populateErrors(final Errors errors, final AddressVerificationResult<AddressVerificationDecision> addressVerificationResult) {
