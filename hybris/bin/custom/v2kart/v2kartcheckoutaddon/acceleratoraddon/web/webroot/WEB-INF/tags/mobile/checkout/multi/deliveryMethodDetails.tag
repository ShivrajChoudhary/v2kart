...
<li class="delivery_method_item">
	<input type="radio" name="delivery_method" id="${deliveryMethod.code}" value="${deliveryMethod.code}" ${isSelected ? 'checked="checked"' : ''} data-role="radio" data-theme = "c"/>
    ...
    <c:if test="${empty deliveryMethod.deliveryCost}">
        <label for="${deliveryMethod.code}">${deliveryMethod.name}&nbsp;-&nbsp;${deliveryMethod.description}&nbsp;-&nbsp;<span>Rs.</span> 0.00</label>
    </c:if>
</li>
