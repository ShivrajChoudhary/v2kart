 ...
 *  
 */
package in.com.v2kart.cockpits.cmscockpit.sitewizard;
...
public class CMSSiteUtils {
    ...
    public static void populateCmsSite(final List<PageTemplateModel> sourceTemplates, final Set<CatalogVersionModel> targetCatalogVersions,
            final ContentCatalogModel contentCatalog, final CMSSiteModel cmsSiteModel, final String homepageName, final String homepageLabel) {
    ...
    /**
     * Changes homepage template if possible. First tries to find first landing page template in selected templates, if none found then
     * looks for first template restricted to contentPages, if not found, homepage template is not changed
     */
    ...
    /**
     * internals for {@link #copyPageTemplatesDeep(List, CatalogVersionModel, ContentCatalogModel)} separated from platform so could be
     * tested separately somehow
     */
    protected static List<PageTemplateModel> copyPageTemplatesDeepImpl(final List<PageTemplateModel> sourceTemplates,
        ...
        for (final PageTemplateModel template : sourceTemplates) {
            ...
            for (final ContentSlotForTemplateModel contentSlotForTemplate : template.getContentSlots()) {
                ...
                clonedContentSlotModel.setUid(getUidWithSuffix(template.getUid(), contentSlotModel.getUid(), catalogId, catalogVersionId));
    ...
    public static void synchronizeCatVersions(final CatalogVersionSyncJobModel catalogVersionSyncJob, final ModelService modelService) {
    ...
    @SuppressWarnings("deprecation")
    protected static SyncItemJob setupStoreTemplateSyncJobs(final CatalogVersionSyncJobModel syncJobModel, final ModelService modelService) {
        ...
        if (syncJob == null) {
            ...
            syncJob.setSyncLanguages(JaloSession.getCurrentSession().getSessionContext(), C2LManager.getInstance().getAllLanguages());
