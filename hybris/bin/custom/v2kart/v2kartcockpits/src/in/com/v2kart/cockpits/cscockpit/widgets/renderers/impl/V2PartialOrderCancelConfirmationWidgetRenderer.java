...
/**
 * Renderer for displaying the confirmation popup for partial cancellation of order.
 *
 * @author pravesh.gupta@nagarro.com
 */
public class V2PartialOrderCancelConfirmationWidgetRenderer extends
    ...
    /**
     * Creates the internal content of the partial cancellation confirmation popup. Displays the refund amount only for confirmation.
     *
    ...
    protected HtmlBasedComponent createContentInternal(
        ...
        final Double partialOrderRefundAmount = widget.getWidgetController().getCancellationRefundAmount(false);
        if (partialOrderRefundAmount != null) {
            ...
            final String refundAmountText = LabelUtils.getLabel(widget, "refundAmount",
                    new Object[] { currencyInstance.format(partialOrderRefundAmount) });
        ...
        confirmButton.addEventListener("onClick", createPartialCancelConfirmedEventListener(widget));
        return container;
    }

    /**
     * Creates the event listener for partial cancellation cofirmation.
     *
     * @param widget
     *        the widget
     ...
     */
    protected EventListener createPartialCancelConfirmedEventListener(
            final InputWidget<DefaultListWidgetModel<TypedObject>, V2CancellationController> widget) {
        final OrderCancellationConfirmedEventListener partialCancellationConfirmedEventListener = new OrderCancellationConfirmedEventListener(
                widget) {

            /**
             * Defines handling of partial cancellation confirmation. Creates a partial order cancellation request using the controller. If
             * successful then closes the popups otherwise shows the problem that occured.
             */
            @Override
            public void handleCancelConfirmedEvent(final InputWidget<DefaultListWidgetModel<TypedObject>, V2CancellationController> widget,
                ...
                try {
                    final TypedObject cancellationRequest = widget.getWidgetController().requestPartialCancellation();
        ...
        return partialCancellationConfirmedEventListener;
    }
