...
public class StoreLocatorPageController extends AbstractSearchPageController {
    ...
    public String findStores(@RequestParam(value = "page", defaultValue = "0") final int page,
            ...
            @RequestParam(value = "sort", required = false) final String sortCode, @RequestParam(value = "q") final String locationQuery,
            ...
            final Model model, final BindingResult bindingResult) throws GeoLocatorException, MapServiceException, CMSItemNotFoundException {
        ...
        if (latitude != null && longitude != null) {
            ...
            setUpMetaDataForContentPage(model, getContentPageForLabelOrId(STORE_FINDER_CMS_PAGE_LABEL));
    ...
    protected void setUpNoResultsErrorMessage(final Model model, final StoreFinderSearchPageData<PointOfServiceData> searchResult,
        ...
        if (searchResult.getResults().isEmpty() && geoPoint.getLatitude() == 0d && geoPoint.getLongitude() == 0d) {
            GlobalMessages.addErrorMessage(model, "storelocator.error.no.results.subtitle");
        } else if (searchResult.getResults().isEmpty()) {
    ...
    protected void setUpSearchResultsForPosition(final GeoPoint geoPoint, final PageableData pageableData, final Model model) {
        ...
        // To get the location search data w.r.t the maximum radius for POS search
        ...
        if (null != baseStore && null != baseStore.getMaxRadiusForPoSSearch()) {
            searchResult = storeFinderFacade.positionSearch(geoPoint, pageableData, baseStore.getMaxRadiusForPoSSearch());
        } else {
            searchResult = storeFinderFacade.positionSearch(geoPoint, pageableData);
        }
        ...
        setUpNoResultsErrorMessage(model, searchResult, newGeoPoint);
    }
    ...
    protected void setUpSearchResultsForLocationQuery(final String locationQuery, final PageableData pageableData, final Model model) {
        // Run the location search & populate the model
        final StoreFinderSearchPageData<PointOfServiceData> searchResult = storeFinderFacade.locationSearch(locationQuery, pageableData);
        final GeoPoint geoPoint = new GeoPoint();
        ...
        setUpNoResultsErrorMessage(model, searchResult, geoPoint);
    }
