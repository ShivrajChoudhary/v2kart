...
import in.com.v2kart.core.search.solrfacetsearch.PriceRangeSolrSearchQueryPostProcessor;
...
public class CategoryPageController extends AbstractSearchPageController {
    ...
    private V2KartProductSearchFacade<ProductData> productSearchFacade;
    ...
    public String category(@PathVariable("categoryCode") final String categoryCode,
        ...
        // handlePriceWithoutRange(searchQuery);
        final CategorySearchEvaluator categorySearch = new CategorySearchEvaluator(categoryCode, XSSFilterUtil.filter(searchQuery), page,
    ...
    public FacetRefinement<SearchStateData> getFacets(@PathVariable("categoryCode") final String categoryCode,
        ...
        final CategoryModel category = commerceCategoryService.getCategoryForCode(categoryCode);
        final CategoryPageModel categoryPage = getCategoryPage(category);
        final CategorySearchEvaluator categorySearch = new CategorySearchEvaluator(categoryCode, searchQuery, page, showMode, sortCode,
                categoryPage);
        categorySearch.doSearch();
    ...
    public String getResults(@PathVariable("categoryCode") final String categoryCode,
            ...
            @RequestParam(value = "page", defaultValue = "0") final int page,
            @RequestParam(value = "show", defaultValue = "Page") final ShowMode showMode,
            @RequestParam(value = "sort", required = false) final String sortCode, final Model model) throws UnsupportedEncodingException {
        final CategoryModel category = commerceCategoryService.getCategoryForCode(categoryCode);
        final CategoryPageModel categoryPage = getCategoryPage(category);
        final CategorySearchEvaluator categorySearch = new CategorySearchEvaluator(categoryCode, searchQuery, page, showMode, sortCode,
                categoryPage);
    ...
    @ResponseBody
    @RequestMapping(value = CATEGORY_CODE_PATH_VARIABLE_PATTERN + "/mobile/results", method = RequestMethod.GET)
    public SearchResultsData<ProductData> getMobileResults(@PathVariable("categoryCode") final String categoryCode,
            @RequestParam(value = "q", required = false) final String searchQuery,
            @RequestParam(value = "page", defaultValue = "0") final int page,
            @RequestParam(value = "show", defaultValue = "Page") final ShowMode showMode,
            @RequestParam(value = "sort", required = false) final String sortCode, final Model model) throws UnsupportedEncodingException {
    ...
    protected <QUERY> void updatePageTitle(final CategoryModel category, final List<BreadcrumbData<QUERY>> appliedFacets, final Model model) {
    ...
    private class CategorySearchEvaluator {
        ...
        public void doSearch() {
            ...
            if (searchQueryData.getValue() == null) {
            	...
            	//filtering the product where Image is not available that product don't display      
        //    	searchPageData =filteredProductImage.filterAllProductMedia( productSearchFacade.categorySearch(categoryCode));
                if (categoryPage != null) {
                ...
                //filtering the product where Image is not available that product don't display      
               // searchPageData = filteredProductImage.filterAllProductMedia( productSearchFacade.categorySearch(categoryCode, searchState, pageableData));
            }
        }
    }
    @SuppressWarnings("unused")
    @RequestMapping(value = CATEGORY_CODE_PATH_VARIABLE_PATTERN + "/populateFacetsAndProducts", method = RequestMethod.GET)
    public String populateFacetsAndProductsOnFacetSelection(@PathVariable("categoryCode") final String categoryCode,
            @RequestParam(value = "q", required = false) final String searchQuery,
            @RequestParam(value = "page", defaultValue = "0") final int page,
            @RequestParam(value = "show", defaultValue = "Page") final ShowMode showMode,
            @RequestParam(value = "sort", required = false) final String sortCode,
        ...
        final CategorySearchEvaluator categorySearch = new CategorySearchEvaluator(categoryCode, searchQuery, page,
                showMode, sortCode, categoryPage);
        categorySearch.doSearch();
        populatePriceRange(categorySearch.getSearchPageData(), model, priceRange);
        if (min == null && max == null && sortCode == null) {
            Map<String, Object> map = model.asMap();
            min = String.valueOf(map.get("categoryMin"));
            max = String.valueOf(map.get("categoryMax"));
        ...
        model.addAttribute("categoryName", category.getName());
        storeCmsPageInModel(model, categorySearch.categoryPage);
        populateModel(model, categorySearch.getSearchPageData(), showMode);
        model.addAttribute("searchPageData", categorySearch.getSearchPageData());

        return "/pages/category/populateFacetsAndProducts";

    }

    protected void populateMyModel(final Model model,
            final ProductCategorySearchPageData<SearchStateData, ProductData, CategoryData> searchPageData, final ShowMode showMode) {
        ...
        model.addAttribute("isShowAllAllowed", Boolean.FALSE);
        model.addAttribute("isShowPageAllowed", calculateShowPaged(searchPageData, showMode));
        model.addAttribute("paginationType", "infiniteScroll");
    }

    private void handlePriceWithoutRange(String searchQuery) {
        if (PriceRangeSolrSearchQueryPostProcessor.priceRange == null && searchQuery != null) {
            priceRange = productSearchFacade.getSolrPriceRange(searchQuery);
            PriceRangeSolrSearchQueryPostProcessor.priceRange = priceRange;
        }
    }
}
