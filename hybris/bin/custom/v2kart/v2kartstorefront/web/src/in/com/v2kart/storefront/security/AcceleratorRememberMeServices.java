...
import in.com.v2kart.storefront.web.wrappers.RemoveEncodingHttpServletRequestWrapper;
...
public class AcceleratorRememberMeServices extends TokenBasedRememberMeServices {
    ...
    @Override
    protected void setCookie(final String[] tokens, final int maxAge, final HttpServletRequest request, final HttpServletResponse response) {
        if (!getCheckoutCustomerStrategy().isAnonymousCheckout()) {
            super.setCookie(tokens, maxAge, new RemoveEncodingHttpServletRequestWrapper(request, getUrlEncodingPattern(request)), response);
        }
    }
