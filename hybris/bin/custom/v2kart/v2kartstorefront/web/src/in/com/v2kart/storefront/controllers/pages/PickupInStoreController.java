...
import in.com.v2kart.storefront.security.cookie.CustomerLocationCookieGenerator;
...
public class PickupInStoreController extends AbstractSearchPageController {
    ...
    @Resource(name="storeFinderSearchPageData")
    ...
    public String getPointOfServiceForStorePickupSubmit(@PathVariable("productCode") final String productCode,
        ...
        return getPointOfServiceForStorePickup(productCode, locationQuery, geoPoint, page, showMode, sortCode, cartPage, entryNumber,
                model, RequestMethod.POST, response);
    }
    ...
    public String getPointOfServiceForStorePickupClick(@PathVariable("productCode") final String productCode,
        ...
        return getPointOfServiceForStorePickup(productCode, location, geoPoint, page, showMode, sortCode, cartPage, entryNumber, model,
    ...
    protected String getPointOfServiceForStorePickup(final String productCode, final String locationQuery, final GeoPoint geoPoint,
        ...
        if (StringUtils.isNotBlank(locationQuery)) {
            ...
            if (RequestMethod.POST.equals(requestMethod) && storeFinderStockSearchPageData.getPagination().getTotalNumberOfResults() > 0) {
    ...
    public String addToCartPickup(@RequestParam("productCodePost") final String code, @RequestParam("storeNamePost") final String storeId,
    ...
    @RequestMapping(value ="/pointOfServices", method = RequestMethod.POST)
    public String getPointOfServiceForPickupClick(@RequestParam(value = "locationQuery", required = false) final String locationQuery,
            @RequestParam(value = "latitude", required = false) final Double latitude,
            @RequestParam(value = "longitude", required = false) final Double longitude,
            @RequestParam(value = "page", defaultValue = "0") final int page,
            @RequestParam(value = "show", defaultValue = "Page") final AbstractSearchPageController.ShowMode showMode,
        ...
        return getPointOfServiceForPickup(locationQuery, geoPoint, page, showMode, sortCode, entryNumber,
                model, RequestMethod.POST, response);
    }
    
    
    @RequestMapping(value ="/pointOfServices", method = RequestMethod.GET)
    public String getPointOfServiceForPickupClick(@RequestParam(value = "page", defaultValue = "0") final int page,
            @RequestParam(value = "show", defaultValue = "Page") final AbstractSearchPageController.ShowMode showMode,
            @RequestParam(value = "sort", required = false) final String sortCode,
            @RequestParam(value = "entryNumber", defaultValue="0") final Long entryNumber,
            final HttpServletResponse response, final Model model) {
        ...
        return getPointOfServiceForPickup(location, geoPoint, page, showMode, sortCode, entryNumber, model,
                RequestMethod.GET, response);
    }
    
    protected String getPointOfServiceForPickup(final String locationQuery, final GeoPoint geoPoint,
            final int page, final AbstractSearchPageController.ShowMode showMode, final String sortCode,
            final Long entryNumber, final Model model, final RequestMethod requestMethod, final HttpServletResponse response) {
        final int pagination = getSiteConfigService().getInt(POINTOFSERVICE_DISPLAY_SEARCH_RESULTS_COUNT, 6);

        StoreFinderSearchPageData<PointOfServiceDistanceData> storeFinderSearchPageData=new StoreFinderSearchPageData<PointOfServiceDistanceData>();

        if (StringUtils.isNotBlank(locationQuery)) {
//            storeFinderStockSearchPageData = storeFinderStockFacade.productSearch(locationQuery, productData,
//                    createPageableData(page, pagination, sortCode, showMode));
            
            storeFinderSearchPageData = getStoreFinderService()
                    .locationSearch(getBaseStoreService().getCurrentBaseStore(), locationQuery, createPageableData(page, pagination, sortCode, showMode));
            model.addAttribute("locationQuery", locationQuery.trim());
            if (RequestMethod.POST.equals(requestMethod) && storeFinderSearchPageData.getPagination().getTotalNumberOfResults() > 0) {
                final UserLocationData userLocationData = new UserLocationData();
...
//                    .positionSearch(getBaseStoreService().getCurrentBaseStore(), geoPoint, pageableData);
             storeFinderSearchPageData = getStoreFinderService()
                    .positionSearch(getBaseStoreService().getCurrentBaseStore(), geoPoint, createPageableData(page, pagination, sortCode, showMode));
            model.addAttribute("geoPoint", geoPoint);
            if (RequestMethod.POST.equals(requestMethod) && storeFinderSearchPageData.getPagination().getTotalNumberOfResults() > 0) {
                final UserLocationData userLocationData = new UserLocationData();
                userLocationData.setPoint(geoPoint);
...
//            storeFinderStockSearchPageData = emptyStoreFinderResult(productData);
            storeFinderSearchPageData.setResults(Collections.<PointOfServiceDistanceData> emptyList());
            storeFinderSearchPageData.setPagination(createEmptyPagination());
        }

        populateModel(model, storeFinderSearchPageData, showMode);
