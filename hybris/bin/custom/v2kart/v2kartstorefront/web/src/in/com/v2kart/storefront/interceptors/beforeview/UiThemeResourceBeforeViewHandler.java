...
import in.com.v2kart.storefront.util.CSRFTokenManager;
import in.com.v2kart.storefront.web.view.UiExperienceViewResolver;
...
/**
 * Interceptor to setup the paths to the UI resource paths in the model before passing it to the view. Sets up the path to the web
 * accessible UI resources for the following: * The current site * The current theme * The common resources All of these paths are qualified
 * by the current UiExperienceLevel
 */
public class UiThemeResourceBeforeViewHandler implements BeforeViewHandler {
    ...
    public void beforeView(final HttpServletRequest request, final HttpServletResponse response, final ModelAndView modelAndView) {
        ...
        modelAndView.addObject("addOnThemeCssPaths", getAddOnThemeCSSPaths(contextPath, themeName, uiExperienceCodeLower, dependantAddOns));
        modelAndView.addObject("addOnJavaScriptPaths", getAddOnJSPaths(contextPath, siteName, uiExperienceCodeLower, dependantAddOns));
    ...
    protected List getAddOnThemeCSSPaths(final String contextPath, final String themeName, final String uiExperience,
        ...
        final String[] propertyNames = new String[] { RESOURCE_TYPE_CSS + ".paths." + uiExperience + "." + themeName };

        return getAddOnResourcePaths(contextPath, addOnNames, propertyNames);
    }

    protected List getAddOnJSPaths(final String contextPath, final String siteName, final String uiExperience, final List<String> addOnNames) {
