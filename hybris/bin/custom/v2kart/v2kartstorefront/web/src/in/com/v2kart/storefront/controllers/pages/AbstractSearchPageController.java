...
public abstract class AbstractSearchPageController extends AbstractPageController {
    ...
    protected static String priceRange = null;

    public static enum ShowMode {
        Page, All
    }

    protected PageableData createPageableData(final int pageNumber, final int pageSize, final String sortCode, final ShowMode showMode) {
    ...
    protected Map<String, FacetData<SearchStateData>> convertBreadcrumbsToFacets(final List<BreadcrumbData<SearchStateData>> breadcrumbs) {
    ...
    protected void populatePriceRange(ProductSearchPageData<SearchStateData, ProductData> searchPageData, Model model, String priceRange) {

        Integer categoryMin = null;
        Integer categoryMax = null;

        if (searchPageData.getStatsInfoList() != null && searchPageData.getStatsInfoList().size() > 0) {
            List<StatsInfo> stats = searchPageData.getStatsInfoList();
            categoryMin = Double.valueOf(stats.get(0).getMin().toString()).intValue();
            categoryMax = Double.valueOf(stats.get(0).getMax().toString()).intValue();
        }

        for (final BreadcrumbData breadcrumbData : searchPageData.getBreadcrumbs()) {
            if ("price".equals(breadcrumbData.getFacetCode())) {
                String facetValue = breadcrumbData.getFacetValueName().replaceAll("\\$", "");
                String[] values = facetValue.split("-");
                // Handle Price Without Range
                if (values.length == 1 && priceRange != null) {
                    values = priceRange.split(" TO ");
                }
                categoryMin = new Double(values[0]).intValue();
                categoryMax = new Double(values[1]).intValue();
                break;
            }
        }

        if (categoryMin != null && categoryMax != null) {
            model.addAttribute("categoryMin", categoryMin);
