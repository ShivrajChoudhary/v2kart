...
public class Functions {
    ...
    public static boolean evaluateRestrictions(final AbstractCMSComponentModel model) {
        ...
        final CmsPageRequestContextData cmsPageRequestContextData = getCMSPageContextService(request).getCmsPageRequestContextData(request);
    ...
    public static <T> T getSpringBean(final HttpServletRequest httpRequest, final String beanName, final Class<T> beanClass) {
        return RequestContextUtils.getWebApplicationContext(httpRequest, httpRequest.getSession().getServletContext()).getBean(beanName,
                beanClass);
    }
