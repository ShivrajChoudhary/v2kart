ACCMOB.menu = {
	...
	bindPolicyButton: function (connectButton)
	{
		if (connectButton.length > 0)
		{
			connectButton.live("click", function (event)
			{
				$.mobile.easydialog({header:$(this).attr('title'),content: $("#policy-popup").html()});
			});
		}
	},
	...
	headerButtonClicked: function (buttonId, popupId)
	{
		...
		var buttonMarginLeft = parseInt(button.css("marginLeft"));
		var popupLeft = buttonLeft + buttonMarginLeft;
		var popup = $(popupId);
		...
		var top = $("#top-nav-bar").outerHeight();
...
$(document).ready(function ()
{
	with (ACCMOB.menu)
	{
		...
		bindPolicyButton($("#policy-button"));
