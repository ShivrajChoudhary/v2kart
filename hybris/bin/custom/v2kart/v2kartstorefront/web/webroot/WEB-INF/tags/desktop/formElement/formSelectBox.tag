...
<%@ attribute name="firstOptionDisable" required="false" type="java.lang.Boolean" %>
			...
			<div class="controls">
				<form:select id="${idKey}" path="${path}" cssClass="${selectCSSClass}" tabindex="${tabindex}">
					<c:if test="${skipBlank == null || skipBlank == false}">
						<option value=""  ${firstOptionDisable?"":"disabled='disabled'"}  ${empty selectedValue ? 'selected="selected"' : ''}><spring:theme code='${skipBlankMessageKey}'/></option>
					</c:if>
