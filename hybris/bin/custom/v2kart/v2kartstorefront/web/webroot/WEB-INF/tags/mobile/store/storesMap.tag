...
<c:if test="${storeSearchPageData ne null and !empty storeSearchPageData.results}">
	...
	<c:forEach var="location" items="${storeSearchPageData.results}" varStatus="status">
		<c:set var="markers" value="${markers}{'latitude':'${location.geoPoint.latitude}','longitude':'${location.geoPoint.longitude}','title': '${location.address.line1}${comma}${location.address.line2}${comma}${location.address.town}${comma}${location.address.postalCode}${comma}${location.address.country.name}'}${ ! status.last ? ',' : ''}"/>
	</c:forEach>
