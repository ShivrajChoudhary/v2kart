...

{
	...
	"cartPopupHtml":	"<spring:escapeBody javaScriptEscape="true">
							...
							<div class="dialog_container">
								...
								<c:if test="${not empty entry.basePrice}">
								<div class="dialog_container-content">
									...
									<ycommerce:testId code="cart_popup_product_summary">
										<div class="ui-grid-a addToCartSummary" data-theme="b">
											...
											<c:if test="${not empty baseOptions.selected and baseOptions.selected.url eq entry.product.url}">
												<c:forEach items="${baseOptions.selected.variantOptionQualifiers}" var="baseOptionQualifier">
													<c:if test="${baseOptionQualifier.qualifier eq 'style' and not empty baseOptionQualifier.value}">
													...
													<c:if test="${baseOptionQualifier.qualifier eq 'size' and not empty baseOptionQualifier.value}">
														<c:if test="${baseOptionQualifier.value ne 'NA' && baseOptionQualifier.value ne 'A'}">
														<div class="ui-block-a prod_size">
															<spring:theme code="mobile.product.variants.size"/>
														</div>
