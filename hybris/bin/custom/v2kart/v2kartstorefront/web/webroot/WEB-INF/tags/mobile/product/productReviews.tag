...
<div data-role="collapsible" data-theme="g" data-content-theme="c" data-collapsed="true" data-icon="arrow-u"
	class="review_detail" id='review_detail'>
	<h3><spring:theme code="text.productreviews"/></h3>
	<p>
	<div id="read_reviews">
		<c:if test="${not empty product.reviews}">
			<ul class="productReviews" data-role="listview" data-theme="c">
				<c:forEach items="${product.reviews}" var="review" varStatus="status">
					<li data-theme="f" ${status.count >3?"style='display: none'":"" } ${status.count >3?"class='hiddenReview'":"" } >
				...
				<li data-theme="c">
					<div class="ui-grid-a">
                        <c:if test="${fn:length(product.reviews) gt 3}">
                            <div class="ui-block-a">
                                <ycommerce:testId code="show_more_reviews_button">
                                    <a href="javascript:void('#')" class="showReviews" data-theme="b" data-role="button">
						...
						<div class="ui-block-b" ${fn:length(product.reviews) le 3 ? "style=width:100%":""}>
							...
							<a href="${encodedUrl}/${product.code}/writeReview" data-role="button" data-theme="b">
								<c:choose>
