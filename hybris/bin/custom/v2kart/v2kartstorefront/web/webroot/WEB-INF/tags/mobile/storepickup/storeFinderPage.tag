...
<div>
	...
	<div id="pickUpInStoreSearchForms">
		<h3 style="text-align: justify;"><spring:theme code="pickup.product.by.store.location" text="Product by store location" /></h3>
			...
			<div data-role="fieldcontain" class="accmob-storeSearch">
				<form:form id="locationQueryStorefinderForm" name="location_query_storefinder_form" action="${pickUpInStoreFormAction}" method="POST">
					<div class="accmob-storeFinderFieldHolder">
						<input id="storelocator-query" name="locationQuery" class="storeSearchBox" type="search" placeholder="<spring:theme code="storelocator.postcode.city.search"/>" data-theme="b">
						...
						<div class="accmob-storeSearch-trigger">
							<button class="form search storeSearchButton" data-role="button" data-theme="b" id="findStoresByQuery">
								<span class="search-icon"><spring:theme code="storeFinder.search" /></span>
							</button>
						</div>
					</div>
				</form:form>
				<div class="line-text-pickupstore clearfix"><span class="black-circle-style"><spring:theme code="storeFinder.line.text"/></span></div>
				<form:form id="nearMeStorefinderForm" name="near_me_storefinder_form" action="${pickUpInStoreFormAction}" method="POST" >
					...
					<input type="hidden" name="entryNumber" value="${entryNumber}"/>
					<input type="hidden" name="cartPage" value="${cartPage}"/>
					<a href="#" id="findStoresNearMe" class="form search findStoresNearMeButton" data-theme="b" data-role="button" data-icon="custom ui-icon-custom-storesearch">
   ...
   <%--  <div class="ui-grid-a returnBtn margin-LR-10">
        <div class="ui-block-a">
            <a href="/cart" data-theme="g" data-role="button" >
                <spring:theme text="return" code="storeFinder.return" />
            </a>
        </div>
    </div>       --%>              
</div>