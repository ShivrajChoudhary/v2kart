...
<div id="reviewError" style="display: none">${reviewFormHasError}</div>
    ...
    <div class="write_review clearfix">
        ...
        <form:form method="post" action="${productReviewActionUrl}" commandName="reviewForm">
            <div class="write_review_container">
                <formElement:formInputBox idKey="review.headline" labelKey="review.headline" path="headline" inputCSS="text"
                    mandatory="true" />
                <formElement:formTextArea idKey="review.comment" labelKey="review.comment" path="comment" areaCSS="textarea"
                    mandatory="true" />

                <spring:bind path="rating">
                    <div class="control-group<c:if test="${not empty status.errorMessages}"> error</c:if>">
                        <label class="control-label"><spring:theme code="review.rating" /><font color="red"> *</font></label>
                        <div id="stars-wrapper" class="controls clearfix">
                            <c:set var="previousRatingSelected">
                                <fmt:formatNumber maxFractionDigits="0" value="${reviewForm.rating}" />
                            </c:set>
                            <c:forEach begin="1" end="5" varStatus="status">
                                <label><img id = "ratingStars_${status.index}" class="ratingStars ${previousRatingSelected ge status.index ?' selected':' no_star'}"
                                    src="${commonResourcePath}/images/jquery.ui.stars.custom.gif"
                                    alt="<spring:theme code="review.rating.alt"/>" /> 
                                    <form:radiobutton path="rating"
                                        value="${status.index}"  checked="${previousRatingSelected eq status.index ?'checked' : ''}" />${status.index}/${status.end}</label>
                            </c:forEach>
                        </div>
            ...
            <button class="positive" type="submit" value="<spring:theme code="review.submit"/>">
