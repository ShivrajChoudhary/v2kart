		...
		<c:forEach items="${galleryImages}" var="container" varStatus="varStatus">
			<div class='imageGallerySlide swipeGallerySlide'
				...
				<div class="imageContainer">
					<img src="${container.zoom.url}" title="${product.name}" alt="${product.name}" width="100%"/>
            </div>
...
<c:if test="${empty galleryImages}">
	<div class="missingProductImage"><theme:image code="img.missingProductImage.product" alt="Missing product image" title="Missing product image"/></div>
</c:if>
