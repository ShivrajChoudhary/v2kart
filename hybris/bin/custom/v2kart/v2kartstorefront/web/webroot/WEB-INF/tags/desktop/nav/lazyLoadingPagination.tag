<!-- Pagination file or PLP and Product Search Page to hide the Pagination Feature  -->
...
<div class="paginationBar ${(top)?"top":"bottom"} clearfix">
	...
	<c:if test="${not empty searchPageData.sorts}">
		<form id="sort_form_page${top ? '1' : '2'}" name="sort_form${top ? '1' : '2'}" method="get" action="${simpleUri}" class="sortForm">
			<label for="sortOptions${top ? '1' : '2'}"><spring:theme code="${themeMsgKey}.sortTitle"/></label>
			<select id="sortOptions${top ? '1' : '2'}" name="sort" class="sortOptions" onchange="submitSortOptions()">
