...
<div class="userLogin">
    ...
    <form:form action="${action}" method="post" commandName="loginForm" name="LoginForm" onsubmit="return loginFormValidate()">
        ...
        <div class="form_field_error-message">
            <input type="checkbox" name="_spring_security_remember_me" id="rememberme"> <label for="rememberme"
                style="font-weight: 300;">Remember Me</label> <a style="float: right;" href="javascript:void(0)"
                data-url="<c:url value='/login/pw/request'/>" class="password-forgotten"><spring:theme code="login.link.forgottenPwd" /></a>
        </div>
</div>
<c:if test="${expressCheckoutAllowed}">
    <div class="expressCheckoutLogin">
        ...
        <div class="expressCheckoutCheckbox clearfix">
            <label for="expressCheckoutCheckbox"><input id="expressCheckoutCheckbox" name="expressCheckoutEnabled" type="checkbox"
                class="form left doExpressCheckout" /> <spring:theme text="I would like to Express checkout"
