...
<head>
    ...
    <!-- google maps via google client ID or API key(by-default) -->
    <c:if test="${!empty googleApiVersion}">
      <c:choose>
         <c:when test="${!empty googleClientId}">
            <script type="text/javascript" src="http://maps.googleapis.com/maps/api/js?v=${googleApiVersion}&amp;
               client=${googleClientId}&amp;sensor=false"
            ></script>
         </c:when>
         <c:otherwise>
             <script type="text/javascript"
                 src="http://maps.googleapis.com/maps/api/js?v=${googleApiVersion}&key=${googleApiKey}&sensor=false"
             ></script>
         </c:otherwise>
      </c:choose> 
    </c:if>
