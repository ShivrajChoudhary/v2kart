...
<%@ attribute name="storeSearchPageData" required="false" type="de.hybris.platform.commerceservices.search.pagedata.SearchPageData" %>
...
<c:if test="${storeSearchPageData ne null and not empty storeSearchPageData.results}">
	<ul data-role="listview">
		<li data-role="list-divider" class="storeNumberLi">
			<div>
				<h2 class="storeNumbers">
					<c:choose>
		...
		<c:forEach items="${storeSearchPageData.results}" var="pos" varStatus="row">
			...
			<li class="store-list-li" data-theme="c">
				<div class="ui-block-a"  style="width: 10%">
					<img class="mapMarker" data-index="${row.count}" onclick='location.href="${posUrl}"'/>
				</div>
				<div class="ui-grid-b"  style="width: 90%">
					<div class="ui-block-b" style="width: 60%;">
						<a href="${posUrl}" class="store_name"><span class="storeName ui-grid-a">${pos.name}</span></a>
					</div>
					<div class="ui-block-c store_dist">
						...
						<ycommerce:testId code="storeFinder_result_address_label">
							<c:if test="${not empty pos.address}">
								<div class="ui-grid-a" data-theme="e">
									<span>${pos.address.line1}</span>
									<br>
									<span>${pos.address.line2}</span>
									<br>
									<span>${pos.address.town}  </span> 
									<br>
									<span>${pos.address.postalCode}  </span> 
									<br >
									<span class="ui-grid-a">${pos.address.phone}</span>
								</div>
							</c:if>
						</ycommerce:testId>
					</div>
