...
<%@ attribute name="currentTab" required="false" type="java.lang.String"%>
...
<c:if test="${searchPageData.pagination.totalNumberOfResults > 0}">
    ...
    <div class="paginationBar ${(top)?"top":"bottom"} clearfix">
        <table style = "width:100%;">
            <tr>
                <td><ycommerce:testId code="searchResults_productsFound_label">
                        <div class="totalResults">
                            <spring:theme code="${themeMsgKey}.totalResults" arguments="${searchPageData.pagination.totalNumberOfResults}" />
                        </div>
                    </ycommerce:testId></td>
                <td><c:if test="${not empty searchPageData.sorts}">
                        <form id="sort_form${top ? '1' : '2'}" name="sort_form${top ? '1' : '2'}" method="get" action="#" class="sortForm" style = "float:left">
                            <label for="sortOptions${top ? '1' : '2'}"><spring:theme code="${themeMsgKey}.sortTitle" /></label> <select
                ...
                <td><c:if test="${(searchPageData.pagination.numberOfPages > 1)}">
