			...
			<c:forEach items="${galleryImages}" var="container" varStatus="varStatus">
				<li>
					<span class="thumb ${(varStatus.index==0)? "active":""}">
						<img src="${container.thumbnail.url}" data-primaryimagesrc="${container.superZoom.url}" data-galleryposition="${varStatus.index}" alt="${container.thumbnail.altText}" title="${container.thumbnail.altText}" data-zoomImgae="${container.superZoom.url}" onload="javascrpt:$('.productImageGallery .jcarousel-skin').jcarousel({vertical : true});"/>	
					</span>
				</li>
			</c:forEach>
		</ul>
...
<%-- 		<a class="productImagePrimaryLink" id="imageLink" href="${productZoomImagesUrl}" data-href="${productZoomImagesUrl}" target="_blank" title="<spring:theme code="general.zoom"/>"> --%>
			<div class="productImagePrimaryLink" id="">
