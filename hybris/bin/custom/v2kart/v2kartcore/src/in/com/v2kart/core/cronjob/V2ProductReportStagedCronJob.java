...
public class V2ProductReportStagedCronJob extends
	AbstractJobPerformable<CronJobModel> {

    private final Logger LOG = Logger
	    .getLogger(V2ProductReportStagedCronJob.class);
	...
	final StringBuilder queryBuilder = new StringBuilder(
		"select {%s} from {%s as p "
			...
			+ "WHERE ( {cv.version} = 'Staged' AND {c.id} = 'v2kartProductCatalog' AND {p.variantType} IS NULL )");
	...
	String filename = fileDir + "Product_Report_Staged.xls";
	...
	if (products != null && !products.isEmpty()) {
	    LOG.info("Total product available in Staged version ["
		    + products.size() + "]");
	    ...
	    // rowhead.createCell(27).setCellValue("PLP IMAGE STATUS ");
	...
	if (productModel.getBaseProduct() instanceof V2kartStyleVariantProductModel) {
		...
		if (CollectionUtils.isNotEmpty(mediaContainers)) {
			...
			// mediaService.getMediaByFormat(mediaContainer,mediaService.getFormat("Product-220Wx275H"));
			// if(media!=null){
			// galleryAvailability=true;
			// }
			// else{
			// galleryAvailability=false;
			// }
			// } catch (final ModelNotFoundException e) {
			// galleryAvailability=false;
			// LOG.debug("Product("+
			// productModel.getCode()+") does not have media with format-'Product-220Wx275H'");
			// }
		    }
		}
