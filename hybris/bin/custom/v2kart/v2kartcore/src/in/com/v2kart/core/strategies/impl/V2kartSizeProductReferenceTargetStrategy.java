 ...
 */
public class V2kartSizeProductReferenceTargetStrategy implements ProductReferenceTargetStrategy {
    ...
    public ProductModel getTarget(final ProductModel sourceProduct, final ProductReferenceModel reference) {
        if (sourceProduct instanceof V2kartSizeVariantProductModel && reference.getTarget() instanceof V2kartStyleVariantProductModel) {
            ...
            if (CollectionUtils.containsAny(sourceSuperCategories, targetSuperCategories)) {
                ...
                final String size = ((V2kartSizeVariantProductModel) sourceProduct).getSize();
                for (final VariantProductModel variant : reference.getTarget().getVariants()) {
                    if (variant instanceof V2kartSizeVariantProductModel
                            && size.equals(((V2kartSizeVariantProductModel) variant).getSize())) {
                        return variant;
                    }
                }
            }
